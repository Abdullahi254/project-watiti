...
function OtherExpTable({ numplate }) {
    ...
    const addOtherExpenseHandler = async (e) => {
        ...
        try {
            ...
            await addDoc(collection(db, `users/${currentUser.uid}/otherExpenses`), {
    ...
    const handleDel = async (name) => {
        ...
        try {
            ...
            if (confirm(text) == true) {
                await deleteDoc(doc(db, `users/${currentUser.uid}/otherExpenses`, `${name}`))
                setLoading(false)
    ...
    const handleSearch = async () => {
        ...
        try {
            if (date1Ref.current.value.length > 0 && date2Ref.current.value.length > 0) {
                ...
                const q = query(collection(db, `users/${currentUser.uid}/otherExpenses`),
    ...
    useEffect(() => {
        setLoading(true)
        setLatest(true)
        const q = query(collection(db, `users/${currentUser.uid}/otherExpenses`), orderBy("date", "desc"), limit(3));
    ...
    useEffect(() => {
        setLoading(true)
        setLatest(true)
        const q = query(collection(db, `users/${currentUser.uid}/otherExpenses`), orderBy("date", "desc"));
        const unsub = onSnapshot(q, (querySnapshot) => {
            setLoading(false)
            if (querySnapshot.docs.length > 1) {
                const initial = 0
