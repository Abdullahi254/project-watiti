...
function FuelTable({ numplate }) {
    ...
    const addFuelExpenseHandler = async (e) => {
        e.preventDefault()
        setError()
        setSuccess()
        setLoading(true)
        try {
            ...
            await addDoc(collection(db, `users/${currentUser.uid}/vehicles/${numplate}/fuel`), {
    ...
    const handleDel = async (name) => {
        ...
        try {
            ...
            if (confirm(text) == true) {
                await deleteDoc(doc(db, `users/${currentUser.uid}/vehicles/${numplate}/fuel`, `${name}`))
                setLoading(false)
    ...
    const handleSearch = async () => {
        ...
        try {
            if (date1Ref.current.value.length > 0 && date2Ref.current.value.length > 0) {
                ...
                const q = query(collection(db, `users/${currentUser.uid}/vehicles/${numplate}/fuel`),
    ...
    useEffect(() => {
        setLoading(true)
        setLatest(true)
        const q = query(collection(db, `users/${currentUser.uid}/vehicles/${numplate}/fuel`), orderBy("date", "desc"), limit(5));
    ...
    return (
        <div className='my-2 w-full border-2 rounded-lg overflow-x-auto relative bg-white py-6 md:px-4 text-xs md:text-sm'>
            <div ref={componentRef}>
                <h2 className='text-xs md:text-sm mb-1 uppercase'>Fuel Expense ({numplate})</h2>
                ...
                <div className='w-full flex justify-start mt-2 px-6 py-4'>
                    {
                        latest ? <p className='italic text-xs md:text-sm underline'>Latest Expenses(5)</p> :
...

            {
                showButton ? <div className='w-full py-2 flex justify-around'>
                    ...
                    <form className="w-full max-w-[400px] mx-auto py-2 flex-wrap" onSubmit={addFuelExpenseHandler}>
